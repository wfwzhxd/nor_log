...
typedef struct
{
    ...
    uint16_t hash;      /* Hash/checksum value for data integrity */
} base_log_entry_t;
 ...
 *
 * This function writes a log entry to flash with proper hash calculation
 ...
 * into the provided buffer. The entry is validated for hash integrity
 ...
 *                (must be at least ctx->sizeof_log_entry bytes)
 * Returns:
 *   true if index is valid and entry has valid hash and log id consistency,
